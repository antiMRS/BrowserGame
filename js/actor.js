...
Actor.prototype.updateVisibility = function() {
	...
	fov.compute(this.pos[0], this.pos[1], Math.ceil(this.vision * dungeon.biome.visionMult), callback.bind(this))
}
...
Actor.prototype.teleportTo = function(x, y) {
	let tyle = world.dungeon.getTile(x, y)
	if (tyle.walkable) {
		console.log(this.name, "telepoting to (", x, ";", y,")")
		this.pos[0] = x
		this.pos[1] = y
	}
}
...
Actor.prototype.doPath = function(checkItems, checkWorldChange) {
	if (this.path.length) {
		...
		var plant = world.dungeon.getPlantOn(waypoint[0], waypoint[1])
		// Checks enemy.
		...
		if (plant) {
			console.info("You stepped on a plant")
			plant.StepOn(this)
			return false
		}
